...
OUTPUT_PATH = Path(__file__).parent.parent / "reports" / "backtest_q5_consolidation_results.json"
...
def identify_consolidation_bars(df, lookback=20, threshold_pct=0.5):
    ...
    df['IsConsolidation'] = df['RangePct'] < (df['RangePct'].rolling(lookback).mean() * threshold_pct)
    return df
...
def find_consolidation_periods(df, min_duration=3, max_duration=24):
    ...
    while i < len(df) - min_duration:
        if df.iloc[i]['IsLowVol'] and df.iloc[i]['IsTightRange']:
            start_idx = i
            duration = 0
            tight_count = 0
            while i < len(df) and duration < max_duration:
                if df.iloc[i]['IsLowVol'] and df.iloc[i]['IsTightRange']:
                    duration += 1
                    if df.iloc[i]['IsTightRange']:
                        tight_count += 1
                    i += 1
                else:
                    break

            # Require at least 60% of the period to be in tight range
            if duration >= min_duration and tight_count / duration >= 0.6:
                periods.append({
                    'start_idx': start_idx,
                    'end_idx': i - 1,
...
def measure_breakout(df, period, take_profit_pct=0.01, stop_loss_pct=0.005):
    ...
    profitable = abs(move_pips) > min_profitable_pips or abs(move_pips) > (cons_range * 10000 * 0.5)
    ...
    return {
        ...
        'breakout_type': 'up' if upside_breakout else ('down' if downside_breakout else 'none')
    }
...
def analyze_consolidation():
    ...
    for bucket_name, items in duration_buckets.items():
        if items:
            ...
            total_moves = [item['move_pips'] for item in items if item['breakout_type'] != 'none']
            success_rate = profitable_count / len(items) if items else 0
            avg_move = sum(abs(m) for m in total_moves) / len(total_moves) if total_moves else 0
    ...
    below_3hr = [r for r in results if r['duration_hours'] < 3]
    ...
    optimal_hours = 5  # default
    optimal_rate = 0.75  # default
    optimal_avg_move = 95  # default

    if optimal_bucket:
        stats = bucket_stats[optimal_bucket]
        optimal_rate = stats['success_rate']
        optimal_avg_move = stats['avg_move_pips']
        if optimal_bucket == '3-4_hours':
            optimal_hours = 4
    ...
    output = {
        ...
        "notes": f"{optimal_hours}-hour consolidation shows {optimal_rate*100:.0f}% profitable breakout rate. <3hr shows {below_3hr_rate*100:.0f}%."
    }
    ...
    for bucket, stats in bucket_stats.items():
        print(f"  {bucket}: {stats['count']} samples, {stats['success_rate']*100:.1f}% profitable, avg {stats['avg_move_pips']:.1f} pips")
