        ...
        if df.iloc[i]['IsLowVol']:
            ...
            while i < len(df) and duration < max_duration:
                ...
                if is_tight_range:
                    tight_count += 1
                i += 1
            if duration >= min_duration and (tight_count / duration if duration > 0 else 0) >= 0.6:
                periods.append({
                    'start_idx': start_idx,
                    'end_idx': i - 1,
...
def analyze_consolidation():
    ...
    output = {
        ...
        "notes": (f"{optimal_hours}-hour consolidation shows {optimal_rate*100:.0f}% profitable breakout rate. <3hr shows {below_3hr_rate*100:.0f}%."
                  if optimal_hours is not None else "No optimal bucket with sufficient samples found.")
    }
