...
def analyze_reversals(
    bars: List[Dict], instrument: str, start_date: str, end_date: str
) -> Dict:
    ...
    avg_wed_pips = (
        sum(r["close_diff"] for r in wed_reversals) / len(wed_reversals)
        if wed_reversals
        else 0
    )
    avg_tue_pips = (
        sum(r["close_diff"] for r in tue_reversals) / len(tue_reversals)
        if tue_reversals
        else 0
    )
    avg_thu_pips = (
        sum(r["close_diff"] for r in thu_reversals) / len(thu_reversals)
        if thu_reversals
        else 0
    )
...
def main():
    ...
    for filename, instrument in instruments:
        ...
        print(f"\n=== {instrument} ===")
        print(
            f"Wednesday: {result['wednesday']['reversals']}/{result['wednesday']['total']} = {result['wednesday']['rate']:.0%} reversal rate, avg {result['wednesday']['avg_pips']:.1f} pips"
        )
        print(
            f"Tuesday: {result['tuesday']['reversals']}/{result['tuesday']['total']} = {result['tuesday']['rate']:.0%}"
        )
        print(
            f"Thursday: {result['thursday']['reversals']}/{result['thursday']['total']} = {result['thursday']['rate']:.0%}"
        )
