...
Methodology:
- Load EURUSD D1 data
- For each day, check if it reversed from the previous trading day's direction
- Only count pairs where weekdays are adjacent (skip holiday gaps)
...
def _are_adjacent_weekdays(prev_dow: int, curr_dow: int) -> bool:
    if curr_dow == 0 and prev_dow == 4:
        return True
    return curr_dow == prev_dow + 1
...
def analyze_reversals(bars: list[Bar]) -> dict:
    ...
    for i in range(1, len(bars)):
        ...
        if not _are_adjacent_weekdays(prev_dow, curr_dow):
            continue
    ...
    return {
        ...
        "methodology": {
            ...
            "weekday_adjacency": "Only adjacent weekday pairs counted (holiday gaps excluded)",
        },
    }
...
def main():
    ...
    output_file = project_root / "reports" / "backtest_q3_wednesday_reversal.json"
