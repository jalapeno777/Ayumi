...
def migrate_file(con: duckdb.DuckDBPyConnection, csv_path: Path) -> dict:
    ...
    log.info(
        "Loading %s (symbol=%s tf=%s suffix=%s)",
        csv_path.name,
        symbol,
        timeframe,
        suffix or "-",
    ...
    if not all([open_c, high_c, low_c, close_c]):
        log.error("[%s] missing OHLC columns; cols=%s", csv_path.name, list(df.columns))
        return {
            "file": csv_path.name,
            "csv_rows": csv_row_count,
            "db_rows": 0,
    ...
    if n_bad:
        log.warning(
            "[%s] dropping %d bad rows (NaN price or unparseable ts)",
            csv_path.name,
            n_bad,
        )
...
def verify_random_samples(con: duckdb.DuckDBPyConnection, n: int = 10, seed: int = 42) -> list[dict]:
    ...
    for sym, tf, ts, o_db, h_db, l_db, c_db in rs:
        ...
        ok = (
            abs(csv_open - o_db) < 1e-5
            and abs(csv_high - h_db) < 1e-5
            and abs(csv_low - l_db) < 1e-5
            and abs(csv_close - c_db) < 1e-5
        )
        results.append(
            {
                "symbol": sym,
