...
class CTraderHistoricalClient:
    ...
    def _resolve_ctid_account_id(self) -> int:
        """Resolve trader_login to internal ctidTraderAccountId via account list API."""
                ...
                for acc in parsed.ctidTraderAccount:
                    if acc.traderLogin == self.trader_login:
                        ...
                        logger.info(
                            ...
                            self.trader_login,
                            acc.ctidTraderAccountId,
                            acc.isLive,
                        )
                        return self._ctid_account_id
                available = [a.traderLogin for a in parsed.ctidTraderAccount]
                raise ValueError(
                    f"trader_login {self.trader_login} not found in account list. "
                    f"Available logins: {available}"
                )
            finally:
                client.stopService()

        return _run_reactor(load)
    ...
    async def _auth(self, client: Client) -> None:
        ...
        ctid_id = self._resolve_ctid_account_id()
    ...
    def _ensure_symbols(self) -> dict[str, int]:
        """Load and cache symbol list {name: symbolId}."""
    ...
    def get_historical_bars(
        ...
        while True:
            bars_page = self._fetch_trendbars(symbol_id, period, current_from, to_ts)
            if not bars_page:
                break
