...
class GridManager:
    ...
    def on_trade_close(
        ...
        trade.pips = pips
        pnl = pips * trade.lot_size * self._config.pip_value * 100000
        trade.pnl = pnl
    ...
    def _check_daily_loss(
        ...
        if self._config.risk.max_daily_loss_pct <= 0:
            return False
        current_equity = self._state.equity_at_start + self._state.total_pnl
        daily_loss_pct = abs(self._state.daily_pnl) / current_equity if current_equity > 0 else 0.0
    ...
    def _close_all_trades(self, close_price: float) -> None:
        if self._state is None:
            return
        now = datetime.now()
