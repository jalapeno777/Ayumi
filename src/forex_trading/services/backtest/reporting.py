...
def generate_comparative_report(
    results: list[BacktestMetrics], strategy_names: list[str]
) -> pd.DataFrame:
    reports = []
    for result, name in zip(results, strategy_names):
        compliance_passed = result.compliance_report.get(
            "violation_count", 0
        ) == 0 and result.compliance_report.get("profit_target_reached", False)
        reports.append(
            PerformanceReport(
                strategy_name=name,
...
def monte_carlo_simulation(result: BacktestMetrics, n_simulations: int = 1000) -> dict:
    ...
    return {
        ...
        "cvar_95": float(
            np.mean(
                simulated_totals[simulated_totals <= np.percentile(simulated_totals, 5)]
            )
        ),
...
def calculate_walk_forward_summary(
    wf_results: list[list[BacktestMetrics]],
    strategy_names: list[str],
    is_results: list[list[BacktestMetrics]] | None = None,
) -> pd.DataFrame:
    summary_data = []
        ...
        summary_data.append(
            {
                ...
                "Avg IS Sharpe": f"{avg_is_sharpe:.2f}"
                if avg_is_sharpe is not None
                else "N/A",
                "Avg OOS Sharpe": f"{avg_oos_sharpe:.2f}"
                if avg_oos_sharpe is not None
                else "N/A",
                "IS/OOS Sharpe Ratio": f"{sharpe_ratio:.2f}"
                if sharpe_ratio is not None
                else "N/A",
                "Avg OOS Max DD": f"{np.mean(oos_max_dds):.2%}"
                if oos_max_dds
                else "N/A",
                "Avg OOS Win Rate": f"{np.mean(oos_win_rates):.2%}"
                if oos_win_rates
                else "N/A",
                "Avg OOS Profit Factor": f"{np.mean(oos_profit_factors):.2f}"
                if oos_profit_factors
                else "N/A",
                "Avg OOS Return": f"{np.mean(oos_total_returns):.2%}"
                if oos_total_returns
                else "N/A",
                "OOS Consistency (%)": f"{sum(1 for r in oos_total_returns if r > 0) / len(oos_total_returns) * 100:.0f}%"
                if oos_total_returns
                else "N/A",
            }
        )
