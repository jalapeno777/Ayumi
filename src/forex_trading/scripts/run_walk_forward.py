...
from forex_trading.services.backtest.engine import (
    BacktestEngine, BacktestConfig, BacktestResult, walk_forward_analysis,
)
...
from forex_trading.strategies.regime_switching_momentum import RegimeSwitchingMomentumStrategy
...
def run_single_combo(pair: str, interval: str, tf_config: dict) -> list[dict]:
    ...
    if data is None or len(data) < 200:
        print(f"  SKIP: Insufficient data ({len(data) if data is not None else 0} bars)")
        return []
    ...
    for strategy in strategies:
        try:
            ...
            if avg_sharpe < GO_CRITERIA["min_avg_oos_sharpe"]:
                go = False
                reasons.append(f"Avg OOS Sharpe {avg_sharpe:.2f} < {GO_CRITERIA['min_avg_oos_sharpe']}")

            if avg_max_dd > GO_CRITERIA["max_avg_oos_max_dd"]:
                go = False
                reasons.append(f"Avg OOS Max DD {avg_max_dd:.2%} > {GO_CRITERIA['max_avg_oos_max_dd']:.0%}")

            if consistency < GO_CRITERIA["min_oos_consistency"]:
                go = False
                reasons.append(f"OOS consistency {consistency:.0f}% < {GO_CRITERIA['min_oos_consistency']:.0f}%")
            ...
            if avg_trades_per_window < GO_CRITERIA["min_avg_trades_per_window"]:
                go = False
                reasons.append(f"Avg trades/window {avg_trades_per_window:.1f} < {GO_CRITERIA['min_avg_trades_per_window']}")
            ...
            print(f"  [{status_icon}] {strategy.name:25s} | Sharpe: {avg_sharpe:+.2f} | "
...
def generate_report(all_results: list[dict]) -> str:
    ...
    lines.append(f"- **Timeframes:** {', '.join(tf['label'] for tf in TIMEFRAMES.values())}")
    ...
    lines.append(f"- **Prop firm rules:** FTMO-compliant (5% daily DD, 10% total DD, 10% profit target)")
    ...
    lines.append(f"\n## GO Strategies")
    if go_results:
        lines.append(f"| Strategy | Pair | Timeframe | OOS Sharpe | OOS Max DD | OOS Return | Win Rate | Profit Factor | Recovery Factor | Consistency | Trades | Avg/Wnd |")
        lines.append(f"|----------|------|-----------|------------|-----------|------------|----------|---------------|----------------|-------------|--------|---------|")
    ...
    lines.append(f"\n## NO-GO Strategies")
    if nogo_results:
        lines.append(f"| Strategy | Pair | Timeframe | Reason | OOS Sharpe | OOS Max DD |")
        lines.append(f"|----------|------|-----------|--------|------------|-----------|")
        for r in sorted(nogo_results, key=lambda x: x["avg_oos_sharpe"], reverse=True):
            ...
            lines.append(f"| {r['strategy']} | {r['pair']} | {r['timeframe']} | {r['reason']} | {sharpe} | {dd} |")
    else:
    ...
    for r in all_results:
        ...
        if name not in strategy_summary:
            strategy_summary[name] = {"go": 0, "nogo": 0, "best_sharpe": -999, "best_pair": ""}
        if r["verdict"] == "GO":
    ...
    lines.append(f"|----------|----------|----------------|-----------|---------|")
    for name, s in sorted(strategy_summary.items(), key=lambda x: x[1]["go"], reverse=True):
        overall = "GO" if s["go"] > 0 else "NO-GO"
        lines.append(f"| {name} | {s['go']} GO / {s['nogo']} NO-GO | {s['best_sharpe']:+.2f} | {s['best_pair']} | **{overall}** |")

    lines.append(f"\n## Recommendations")
    if not go_results:
        lines.append(f"\n**No strategies currently meet GO/NO-GO criteria.**")
        lines.append(f"\nRecommended next steps:")
        lines.append(f"1. **Strategy iteration:** All strategies need parameter optimization or redesign")
        lines.append(f"2. **Consider ICT approach:** As noted in Sprint 2 planning, ICT-based strategies may outperform")
        lines.append(f"3. **Data quality:** Validate data quality; premium H1/H4 feeds would improve signal accuracy")
        lines.append(f"4. **Position sizing:** Current engine uses fixed 0.1 lots; integrate Kelly/fixed-fractional sizing")
        lines.append(f"5. **Exit logic:** Strategies lack explicit take-profit/stop-loss levels; add risk-based exits")
    else:
        ...
        lines.append(f"3. Run Monte Carlo simulation on GO strategies for drawdown confidence intervals")

    lines.append(f"\n## Methodology Notes")
    lines.append(f"- Walk-forward uses rolling (not expanding) training windows")
    lines.append(f"- Each OOS window is tested on a fresh engine instance (no state leakage)")
    lines.append(f"- Carry trade strategy uses hardcoded interest rate differentials (Apr 2026 rates)")
    lines.append(f"- Regime-aware strategy uses built-in RegimeClassifier (no external regime input)")
    lines.append(f"- Execution includes pair-specific spreads and random slippage simulation")
    lines.append(f"- Prop firm rules enforced: 5% daily DD, 10% total DD, max 2 concurrent positions, max 10 daily trades")
...
def main():
    ...
    report_path = Path(__file__).resolve().parent.parent.parent / "reports" / "walk_forward_validation.md"
