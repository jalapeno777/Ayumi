...
def main():
    ...
    with repo._connect() as conn:
        ...
        free_users = conn.execute("SELECT COUNT(*) as count FROM users WHERE subscription_tier = 'free'").fetchone()[
            "count"
        ]
        pro_users = conn.execute("SELECT COUNT(*) as count FROM users WHERE subscription_tier = 'pro'").fetchone()[
            "count"
        ]
        vip_users = conn.execute("SELECT COUNT(*) as count FROM users WHERE subscription_tier = 'vip'").fetchone()[
            "count"
        ]
    ...
    with repo._connect() as conn:
        ...
        paid_payouts = conn.execute("SELECT COUNT(*) as count FROM referral_payouts WHERE status = 'paid'").fetchone()[
            "count"
        ]
    ...
    signals_this_week = [
        s for s in signals if datetime.fromisoformat(s["signal_time"]) >= datetime.now(timezone.utc) - timedelta(days=7)
    ]
