...
from src.forex_trading.services.backtest.engine import BacktestEngine, BacktestConfig
...
def test_backtest_engine_initialization():
    config = BacktestConfig(starting_balance=10000.0)
    engine = BacktestEngine(config)
    assert engine.config.starting_balance == 10000.0
...
def test_backtest_engine_run():
    ...
    assert isinstance(result, type(BacktestEngine().run(data, strategy)))
