"""Tests for BACKTEST Q7 Multi-Session M/W Formation Frequency"""
...
class TestSwingPointDetection:
    ...
    def test_detects_swing_highs_and_lows(self):
        ...
        from scripts.backtest_q7_multi_session_mw import detect_swing_points
    ...
    def test_no_swings_in_flat_market(self):
        """No swing points detected in flat/ranging market."""
        from scripts.backtest_q7_multi_session_mw import detect_swing_points
        ...
        points = detect_swing_points(bars, lookback=5)
        assert len(points) == 0
...
class TestMWFormationDetection:
    ...
    def test_formation_detection_runs(self):
        ...
        from scripts.backtest_q7_multi_session_mw import detect_swing_points, find_mw_formations
    ...
    def test_multi_session_detection(self):
        """Should detect formations spanning multiple sessions."""
        from scripts.backtest_q7_multi_session_mw import detect_swing_points, find_mw_formations

        bars = [
            Bar(time=datetime(2024, 1, 1, 9, 0), open=1.10, high=1.10, low=1.10, close=1.10),
            Bar(time=datetime(2024, 1, 1, 11, 0), open=1.08, high=1.08, low=1.08, close=1.08),
            Bar(time=datetime(2024, 1, 1, 12, 0), open=1.10, high=1.10, low=1.10, close=1.10),
            Bar(time=datetime(2024, 1, 1, 14, 0), open=1.07, high=1.07, low=1.07, close=1.07),
            Bar(time=datetime(2024, 1, 1, 15, 0), open=1.09, high=1.09, low=1.09, close=1.09),
            Bar(time=datetime(2024, 1, 1, 16, 0), open=1.08, high=1.08, low=1.08, close=1.08),
            Bar(time=datetime(2024, 1, 1, 17, 0), open=1.11, high=1.11, low=1.11, close=1.11),
            Bar(time=datetime(2024, 1, 1, 18, 0), open=1.06, high=1.06, low=1.06, close=1.06),
        ]

        swing_points = detect_swing_points(bars, lookback=2)
        formations = find_mw_formations(bars, swing_points, min_swing_pct=0.005)
        multi_session = [f for f in formations if f.is_multi_session]
        assert isinstance(multi_session, list)
...
class TestATRCalculation:
    ...
    def test_calculates_atr(self):
        ...
        from scripts.backtest_q7_multi_session_mw import calculate_atr

        bars = [
            Bar(time=datetime(2024, 1, i % 28 + 1, 10, 0), open=1.10, high=1.11, low=1.09, close=1.10)
            for i in range(16)
        ]
...
class TestQ7Results:
    """Integration test for Q7 analysis."""
    ...
    def test_results_format(self):
        ...
        from scripts.backtest_q7_multi_session_mw import analyze_multi_session_mw

        results = analyze_multi_session_mw(bars)

        assert results["question"] == "Q7"
        assert "test_period" in results
        assert results["instrument"] == "EURUSD"
        assert results["timeframe"] == "H1"
        assert results["sample_size"] > 0
        assert "multi_session_pct" in results["results"]
        assert "single_session_pct" in results["results"]
        assert "multi_session_win_rate" in results["results"]
        assert "single_session_win_rate" in results["results"]
        assert "avg_sessions_per_formation" in results["results"]
