...
class TestFindDotenv(unittest.TestCase):
    ...
    def _make_tmp_dir(self) -> tempfile.TemporaryDirectory:
        tmp = tempfile.TemporaryDirectory()
        self._tmp_dirs.append(tmp)
        return tmp

    def test_explicit_dotenv_path_overrides_search(self):
        tmp = self._make_tmp_dir()
        env_file = Path(tmp.name) / ".env"
        env_file.write_text("CTRADER_HOST=explicit\n")
        os.environ["DOTENV_PATH"] = str(env_file)
        result = _find_dotenv()
        self.assertEqual(result, env_file)

    def test_explicit_dotenv_path_returns_none_when_missing(self):
        os.environ["DOTENV_PATH"] = "/nonexistent/path/.env"
        self.assertIsNone(_find_dotenv())

    def test_project_root_dotenv_found(self):
        tmp = self._make_tmp_dir()
        env_file = Path(tmp.name) / ".env"
        env_file.write_text("CTRADER_HOST=project\n")
        with patch("ctrader.connection.__file__", str(Path(tmp.name, "a", "b", "c", "connection.py"))):
            result = _find_dotenv()
            self.assertEqual(result, env_file)

    def test_parent_dir_dotenv_fallback(self):
        parent_tmp = self._make_tmp_dir()
        parent_env = Path(parent_tmp.name) / ".env"
        parent_env.write_text("CTRADER_HOST=parent\n")
        project_dir = Path(parent_tmp.name) / "project"
        project_dir.mkdir()
        with patch("ctrader.connection.__file__", str(project_dir / "a" / "b" / "c" / "connection.py")):
            result = _find_dotenv()
            self.assertEqual(result, parent_env)

    def test_returns_none_when_no_dotenv_found(self):
        tmp = self._make_tmp_dir()
        with patch("ctrader.connection.__file__", str(Path(tmp.name, "a", "b", "c", "connection.py"))), \
             patch("ctrader.connection.Path.cwd", return_value=Path("/tmp")):
            self.assertIsNone(_find_dotenv())
