...
class TestGroupByWeekday:
    def test_groups_correctly(self):
        from scripts.backtest_q3_wednesday_reversal import group_by_weekday
        ...
        bars = [
            Bar(time=mon, open=1.0, high=1.01, low=0.99, close=1.005),
            Bar(time=tue, open=1.0, high=1.01, low=0.99, close=0.995),
            Bar(time=wed, open=1.0, high=1.01, low=0.99, close=1.005),
        ]
        groups = group_by_weekday(bars)
        assert 0 in groups and len(groups[0]) == 1
        assert 1 in groups and len(groups[1]) == 1
        assert 2 in groups and len(groups[2]) == 1
...
class TestQ3Integration:
    ...
    def test_report_json_exists(self):
        report = Path(project_root / "reports/backtest_q3_wednesday_reversal.json")
