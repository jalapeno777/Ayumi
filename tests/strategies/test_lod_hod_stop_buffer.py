...
def _make_bars_same_day(
    n=10, base_price=1.1000, day_high_extra=0.001, day_low_extra=0.001
):
    bars = []
...
class TestApplyLodHodStopBuffer(unittest.TestCase):
    def test_long_stop_pushed_below_lod_with_buffer(self):
        ...
        result = apply_lod_hod_stop_buffer(
            stop_too_close, TradeDirection.LONG, bars, buffer_pips
        )
    ...
    def test_long_stop_already_below_lod_buffer_unchanged(self):
        ...
        result = apply_lod_hod_stop_buffer(
            stop_far_below, TradeDirection.LONG, bars, buffer_pips
        )
    ...
    def test_short_stop_pushed_above_hod_with_buffer(self):
        ...
        result = apply_lod_hod_stop_buffer(
            stop_too_close, TradeDirection.SHORT, bars, buffer_pips
        )
    ...
    def test_short_stop_already_above_hod_buffer_unchanged(self):
        ...
        result = apply_lod_hod_stop_buffer(
            stop_far_above, TradeDirection.SHORT, bars, buffer_pips
        )
    ...
    def test_zero_buffer_returns_original_stop(self):
        ...
        stop = 1.0950
        result = apply_lod_hod_stop_buffer(
            stop, TradeDirection.LONG, bars, buffer_pips=0.0
        )
    ...
    def test_negative_buffer_returns_original_stop(self):
        ...
        stop = 1.0950
        result = apply_lod_hod_stop_buffer(
            stop, TradeDirection.LONG, bars, buffer_pips=-5.0
        )
        self.assertAlmostEqual(result, stop, places=6)
