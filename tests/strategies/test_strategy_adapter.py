...
class TestStrategyAdapter:
    ...
    def test_valid_signal(self, adapter):
        out = adapter.adapt_signal("srmr", {
            "symbol": "EURUSD", "direction": "LONG",
            "entry_price": 1.0800, "stop_loss": 1.0790,
            "take_profit": 1.0850, "confidence": 0.85,
        })
    ...
    def test_missing_direction_raises(self, adapter):
        with pytest.raises(ValueError, match="direction"):
            adapter.adapt_signal("s1", {
                "symbol": "EURUSD", "entry_price": 1.0, "stop_loss": 0.99,
            })

    def test_missing_entry_price_raises(self, adapter):
        with pytest.raises(ValueError, match="entry_price"):
            adapter.adapt_signal("s1", {
                "symbol": "EURUSD", "direction": "LONG", "stop_loss": 0.99,
            })

    def test_missing_stop_loss_raises(self, adapter):
        with pytest.raises(ValueError, match="stop_loss"):
            adapter.adapt_signal("s1", {
                "symbol": "EURUSD", "direction": "LONG", "entry_price": 1.0,
            })

    def test_optional_defaults(self, adapter):
        out = adapter.adapt_signal("s1", {
            "symbol": "GBPUSD", "direction": "SHORT",
            "entry_price": 1.2700, "stop_loss": 1.2720,
        })
