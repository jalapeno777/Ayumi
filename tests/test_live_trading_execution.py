...
class TestSessionRangeMRStrategy:
    def _make_bar(self, hour: int, minute: int = 0, open_p=1.3000, high_p=1.3010, low_p=1.2990, close_p=1.3005) -> Bar:
        return Bar(
    ...
    def test_signal_generation_long(self):
        ...
        if signal is not None:
            ...
            assert signal.take_profit_1 > signal.entry_price if signal.direction == TradeDirection.LONG else signal.take_profit_1 < signal.entry_price
...
class TestSignalEvaluation:
    ...
    def test_evaluate_strategy_handles_low_confidence(self):
        ...
        executor._paper_trader.process_signal.return_value = MagicMock(success=False, rejection_reason="confidence_too_low")
...
class TestQuantPipelineIntegration:
    ...
    def test_pre_trade_check_with_ftmo_config(self):
        ...
        assert decision.action in (TradeAction.ACCEPT, TradeAction.RESIZE, TradeAction.REJECT)
