"""TTC Forward Test Runner — H1 bars, XAUUSD via cTrader live market data.

Launches a ForwardTestEngine with the TTC signal engine for XAUUSD H1.
Runs alongside the existing SRMR+ forward test — separate screen session.

Usage:
  PYTHONPATH=src/forex-bot:src python -m run_ttc_forward
  PYTHONPATH=src/forex-bot:src python -m run_ttc_forward --paper-only

To run alongside SRMR+:
  screen -dmS ttc-forward bash -c 'cd /home/TacoPants/projects/Ayumi/worktrees/kai && PYTHONPATH=src/forex-bot:src PYTHONUNBUFFERED=1 python3 -u -m run_ttc_forward 2>&1 | tee logs/ttc/forward_test_\$(date +%Y%m%d_%H%M%S).log'
"""
...
class TTCForwardTest:
    ...
    def _build_engines(self):
        ...
        # TTC on XAUUSD H1 — primary forward test target
        ttc_strategy = TTCSignalForwardStrategy(
            instrument="XAUUSD",
            timeframe="H1",
            min_confidence=0.40,
            rr_ratio=3.0,
        ...
        engine = ForwardTestEngine(
            config=config,
            strategies=[ttc_strategy],
            ftmo_config=ftmo,
            credentials=creds,
        )
        self._engines.append(PairEngine(symbol="XAUUSD", engine=engine))
        logger.info("Engine built: XAUUSD H1 (TTC)")

    def _on_signal_traded(self, signal):
        logger.info(
            f"TTC SIGNAL: {signal.direction.value} {signal.symbol} @ {signal.entry_price:.5f} "
    ...
    def _status_loop(self):
        while not self._shutdown:
            ...
            for pe in self._engines:
                ...
                bars = health.bars_built
                connected = "Y" if health.connected else "N"
                parts.append(
                    f"{pe.symbol}: bal=${bal:.2f} trades={trades} "
                    f"signals={signals} ticks={ticks} bars={bars} conn={connected}"
                )
            print(" | ".join(parts))
    ...
    def run(self):
        ...
        for pe in self._engines:
            ...
            t = threading.Thread(
                target=self._run_engine, args=(pe,), daemon=True, name=f"ttc-ft-{pe.symbol}"
            )
            pe.thread = t
            t.start()
            pe.running = True
        ...
        self._status_thread = threading.Thread(
            target=self._status_loop, daemon=True, name="ttc-status"
        )
        self._status_thread.start()

        print("TTC forward test started. Press Ctrl+C to stop.")
        print("-" * 60)
    ...
    def _shutdown_handler(self, signum=None, frame=None):
        print("\nShutting down TTC forward test...")
        self._shutdown = True
        for pe in self._engines:
            if pe.running:
                pe.engine.stop()
        ...
        for pe in self._engines:
            ...
            print(f"  {pe.symbol}: bal=${bal:.2f} trades={trades} pnl=${pnl:.2f} "
                  f"ticks={health.ticks_received} signals={health.signals_generated} "
                  f"bars={health.bars_built}")
...
def main():
    ...
    parser = argparse.ArgumentParser(description="TTC Forward Test (XAUUSD H1)")
    parser.add_argument(
        "--paper-only",
        action="store_true",
    ...
    system = TTCForwardTest(paper_only=args.paper_only)
    system.run()
